# Hi, I'm Eyad Haddad 👋

🎓 Computer Science Student  
💻 Interested in Software Development, Cybersecurity, DevOps & AI

## 🛠️ Technologies

- Python
- Java
- C#
- JavaScript
- SQL
- Linux
- Docker
- Git & GitHub

## 🚀 Projects

| Project | Languages / Technologies | Description | Status |
|---|---|---|---|
| 🔐 **File Parser Fuzzing Lab** | C, AFL++, ASan, GDB, CodeQL, Linux | C file parser fuzzing lab for discovering and analyzing memory vulnerabilities using AFL++ | 🔄 In Progress |
| ✋ **Hand-Tracking Math Quiz** | Python, OpenCV, MediaPipe, Tkinter | Gesture-controlled math quiz with real-time hand tracking, timed questions, randomized exams and answer review | ✅ Completed |
| ⌨️ **Hand-Tracking Virtual Keyboard** | Python, OpenCV, MediaPipe, cvzone | Virtual keyboard controlled through real-time hand tracking and computer vision | ✅ Completed |
| 🎮 **Smart Gomoku** | C#, .NET, WinForms, EF Core, SQLite | Gomoku game with AI opponents, multiple difficulty levels and database integration | ✅ Completed |
| 🏧 **ATM System** | Python, Tkinter, SQLite | Desktop ATM simulator with GUI and database integration | ✅ Completed |
| 🔊 **Text-to-Speech** | JavaScript, HTML, CSS | Web application that converts text to speech in multiple languages | ✅ Completed |
| 🐍 **Snake Game** | Java | Classic Snake game demonstrating OOP and GUI programming | ✅ Completed |
| ✊ **Rock Paper Scissors** | JavaScript, HTML, CSS | Rock-Paper-Scissors game against the computer | ✅ Completed |
| 🐦 **Flappy Bird** | JavaScript, HTML, CSS | Browser-based Flappy Bird game | ✅ Completed |
| 🧮 **Calculator** | JavaScript, HTML, CSS | Interactive web calculator | ✅ Completed |

## 📫 Connect with Me

- LinkedIn: www.linkedin.com/in/eyad-haddad
- GitHub: @eyadHaddad98
